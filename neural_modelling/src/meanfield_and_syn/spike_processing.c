...
static inline void setup_synaptic_dma_write(
    ...
    log_info("Writing back %u bytes of plastic region to %08x for spike %u",
...
static void multicast_packet_pl_received_callback(uint key, uint payload) {
    ...
    log_info("Received spike %x with firing_rate %d at %d, DMA Busy = %d",
...
static void dma_complete_callback(UNUSED uint unused, uint tag) {
    ...
    uint32_t n_spikes = dma_n_spikes;// is set to zero
...
void user_event_callback(UNUSED uint unused0, UNUSED uint unused1) {
    ...
    if (buffer_being_read < N_DMA_BUFFERS) {
        ...
        //log_info("if not null");//remove it in futur
    } else {
        // If the DMA buffer is invalid, just do the first transfer possible
        //log_info("dma null");//remove it in futur
